...
company=pd.read_csv("C:\\Users\\hardi\\OneDrive\\Documents\\Excelr_1\\Python\\Random Forest\\Assignments\\Company_Data.csv")
...
company.tail(10)
colnames = company.columns
len(colnames[0:10])
X = company[colnames[0:10]]
labels=['1','2']
bins=[0,10,17]
company['Sales']=pd.cut(company['Sales'],bins=bins,labels=labels)
Y = company[colnames[10]]
from sklearn.ensemble import RandomForestClassifier
rfmodel = RandomForestClassifier(n_jobs=6,oob_score=True,n_estimators=150,criterion="entropy")
rfmodel.fit(X,Y) # Error Can not convert a string into float means we have to use LabelEncoder()
...
rfmodel.oob_score_  # 0.99333
rfmodel.predict(X)
X["rf_pred"] = rfmodel.predict(X)
pd.crosstab(Y,X['rf_pred'])
from sklearn.metrics import confusion_matrix
confusion_matrix(Y,X['rf_pred']) # Confusion matrix
# Accuracy
print ("Accuracy",(121+262)/(121+262+8+9)) #95.75
 ###################################################################

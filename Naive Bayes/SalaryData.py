...
salary_train = pd.read_csv("C:\\Users\\hardi\\OneDrive\\Documents\\Excelr_1\\Python\\Naive Bayes\Assignments\\SalaryData_Train.csv")
...
salary_test=pd.read_csv("C:\\Users\\hardi\\OneDrive\\Documents\\Excelr_1\\Python\\Naive Bayes\Assignments\\SalaryData_Test.csv")
salary_test.columns
...
##sns.tools.plotting.scatter_matrix(salary_test) ##-> also used for plotting all in one graph
...
salary_train['workclass']=lb_make.fit_transform(salary_train['workclass'])
salary_train['education']=lb_make.fit_transform(salary_train['education'])
salary_train['maritalstatus']=lb_make.fit_transform(salary_train['maritalstatus'])
salary_train['occupation']=lb_make.fit_transform(salary_train['occupation'])
salary_train['relationship']=lb_make.fit_transform(salary_train['relationship'])
salary_train['race']=lb_make.fit_transform(salary_train['race'])
salary_train['sex']=lb_make.fit_transform(salary_train['sex'])
salary_train['native']=lb_make.fit_transform(salary_train['native'])
...
print("Accuracy",(10891+780)/(10891+780+2920+780))  # 76%
#####Feature Engineering####################################################
####Imputation
threshold = 0.7
...
bins=[0,10,16.27]
salary_train['Sales']=pd.cut(salary_train['Sales'],bins=bins,labels=labels)
...
forest_test(X_ae, Y)
##################################################################################
salary_train.Salary.value_counts().plot(kind="bar")
from sklearn.model_selection import train_test_split
##from keras.models import Sequential
##train,test = train_test_split(salary_train,test_size = 0.3,random_state=42)
salary_train = pd.read_csv("C:\\Users\\hardi\\OneDrive\\Documents\\Excelr_1\\Python\\Naive Bayes\Assignments\\SalaryData_Train.csv")
salary_test = pd.read_csv("C:\\Users\\hardi\\OneDrive\\Documents\\Excelr_1\\Python\\Naive Bayes\Assignments\\SalaryData_Test.csv")
trainX = salary_train.drop(["Salary"],axis=1)
trainY = salary_train["Salary"]
...
sgnb = GaussianNB()
smnb = MultinomialNB()
trainX['workclass']=LabelEncoder().fit_transform(salary_train['workclass'])
trainX['education']=LabelEncoder().fit_transform(salary_train['education'])
trainX['maritalstatus']=LabelEncoder().fit_transform(salary_train['maritalstatus'])
trainX['occupation']=LabelEncoder().fit_transform(salary_train['occupation'])
trainX['relationship']=LabelEncoder().fit_transform(salary_train['relationship'])
trainX['race']=LabelEncoder().fit_transform(salary_train['race'])
trainX['sex']=LabelEncoder().fit_transform(salary_train['sex'])
trainX['native']=LabelEncoder().fit_transform(salary_train['native'])
testX['workclass']=LabelEncoder().fit_transform(salary_test['workclass'])
testX['education']=LabelEncoder().fit_transform(salary_test['education'])
testX['maritalstatus']=LabelEncoder().fit_transform(salary_test['maritalstatus'])
testX['occupation']=LabelEncoder().fit_transform(salary_test['occupation'])
testX['relationship']=LabelEncoder().fit_transform(salary_test['relationship'])
testX['race']=LabelEncoder().fit_transform(salary_test['race'])
testX['sex']=LabelEncoder().fit_transform(salary_test['sex'])
testX['native']=LabelEncoder().fit_transform(salary_test['native'])
spred_gnb = sgnb.fit(trainX,trainY).predict(testX)
confusion_matrix(testY,spred_gnb)

...
with open("C:\\Users\\hardi\\OneDrive\\Documents\\Excelr_1\\Python\\Association Rules\\my_movies.csv") as f:
    movies = f.read()
##########################################################    
...
frequent_itemsets = apriori(X, min_support=0.10, max_len=3,use_colnames = True)
# Most Frequent item sets based on support 
frequent_itemsets.sort_values('support',ascending = False,inplace=True)
support = frequent_itemsets.iloc[:,0]
itemsets = frequent_itemsets.iloc[:,1]
plt.bar(x=itemsets[0:11],height = support[0:11],width=0.5,align='center',color='rgmyk');plt.xticks(list(range(0,11),),itemsets[0:11]);plt.xlabel('item-sets')
plt.ylabel('support')
